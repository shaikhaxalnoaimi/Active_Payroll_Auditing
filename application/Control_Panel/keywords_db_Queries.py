...
def Fetch_Lable_Keywords(file_name):
    ...
    # conn, cur = createConnection()
    # Define query
    # query = 'SELECT DISTINCT LABEL FROM KEWORDS WHERE FILE_NAME=?'
    # Execute query with parameter
    # labels = db.session.execute(query, (file_name,))
    # Fetch results as DataFrame
    # labels = pd.DataFrame(labels.fetchall(), columns=['LABEL'])

    labels = keywords.query.filter_by(File_name=file_name).all()
    print(labels)
