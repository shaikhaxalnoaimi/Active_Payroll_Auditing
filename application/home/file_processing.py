...
from application.Control_Panel.keywords_db_Queries import Fetch_keywords, Fetch_keywords_Without_Label
...
def Confirm_conflict(new,folder_name):
    ...
    keys = Fetch_keywords_Without_Label(file_name)
...
def Cleaning_Prepar_df(new,approved_columns,folder_name):
    ...
    for column in ColmunsList:
        # (column)
        keys = Fetch_keywords(column, file_name)
