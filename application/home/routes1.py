...
@blueprint.route('/nao_payroll/conflict', methods=["POST", "GET"])
@login_required
def conflict():
...
@blueprint.route('/nao_payroll/conflict2', methods=["POST", "GET"])
@login_required
def conflict2():
    # variable Initilization
    RecivedMessage, file_path, conflict_list, missing_columns, received_value, error, auditing_button, df, key, value, conflict_sheet, missing_list, \
...
@blueprint.route('/nao_payroll/index', methods=["POST", "GET"])
@login_required
def index():
...
@blueprint.route('/nao_payroll/auditingProcess', methods=["POST", "GET"])
@login_required
def auditingProcess():
    value = ''
    error1 = False
...
######################################################################################-->
@blueprint.route('/nao_payroll/managefile', methods=["POST", "GET"])
@login_required
def managefile():
    ...
    dic_list2 = {files_master: 'master_data' for files_master in files1}
    dic_list2.update({k: "MonthlyPayroll_data" for k in files2})
    print("dic_list2", dic_list2)
    print("full_paths1", full_paths1)
    print("files1", files1)
    print("full_paths2", full_paths2)

    full_paths = files1 + files2
    ...
    if request.method == 'POST':
        ...
        if request.form.getlist('checkbox'):
            ...
            print(file_Names1)
            for file_names in file_Names1:

                file_type=dic_list2.get(file_names)
                print(file_type)
                if file_type=="master_data":
                    file_path = secure_join("application", "data", "master_data", user_name, file_names)
                else:
                    file_path = secure_join("application", "data", "MonthlyPayroll_data", user_name, file_names)
    ...
    return render_template('home/managefiles.html', segment="managefile",
                           ...
                           full_paths1=files1,  # master files
                           full_paths2=files2,  # payroll
...
######################################################################################-->
@blueprint.route('/nao_payroll/downloadAuditedEmployeeInfo', methods=['GET', 'POST'])
@login_required
...
######################################################################################-->
@blueprint.route('/nao_payroll/downloadAuditedDeduction', methods=['GET', 'POST'])
@login_required
@csrf.exempt
...
######################################################################################-->
@blueprint.route('/nao_payroll/downloadAuditedAllowanmce', methods=['GET', 'POST'])
@login_required
@csrf.exempt
...
######################################################################################-->
@blueprint.route('/nao_payroll/downloadAuditedEmployeeOvertime', methods=['GET', 'POST'])
@login_required
@csrf.exempt
...
@blueprint.route('/nao_payroll/<template>')
@login_required
