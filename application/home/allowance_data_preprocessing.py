    ...
    if df.columns.nlevels > 1:
        ...
        for Grade in range(0, rows):
            ...
            for allowance_type in range(0, cols - 1):
                ...
                dict = [{'Level':gradeNumber, 'AllowanceType':col_name[j][0], 'Status':col_name[j][1], 'Allowance_Values':df.iloc[Grade, allowance_type], 'Grade_Letter':"","Grade":""}]
                allowance_df = pd.concat([allowance_df, pd.DataFrame.from_records(dict)])
                j = j + 1
        ...
        for Grade in range(0, rows):
            ...
            for allowance_type in range(0, cols - 1):
                ...
                # print(gradeNumber, col_name[j][0], col_name[j][1], df.iloc[Grade, allowance_type])
                dict = [{'Level': gradeNumber, 'AllowanceType': col_name[allowance_type], 'Allowance_Values': df.iloc[Grade, allowance_type], 'Grade_Letter': "", "Grade": ""}]
                allowance_df = pd.concat([allowance_df, pd.DataFrame.from_records(dict)])
...
def Fetch_Allow_SheetNames():
    ...
    allowance_fnames = [sheet for sheet in xls_file.sheet_names]
