"""
File Name: application\home\data_processing.py

Purpose: This file contains functions for data preprocessing and auditing of employee information and deduction in payroll data.
...
Define functions for data preprocessing and auditing:
Organize_Data_View: to organize data view
Check_Duplicate_Name: to check for duplicate employee names
Check_Missing_Name: to check for missing employee names
Check_Invalid_CPR: to check for invalid CPR numbers
Check_Duplicate_CPR: to check for duplicate CPR numbers
Check_Duplicate_Accont_No: to check for duplicate bank account numbers
Check_Invalid_Account_No: to check for invalid bank account numbers
Check_Age: to check for invalid employee ages
Check_Annual_leave: to check for invalid annual leave days
Check_Sick_leave: to check for invalid sick leave days
Check_High_Rank_Posisiton: to check for duplicate high ranking positions
Missing_GRADE: to check for missing employee grades
Missing_MARITAL_STATUS: to check for missing employee marital status
Missing_Employee: to check for missing employees in payroll or master data
Check_Pension_Allowance: to check for invalid pension allowance deduction
Check_Social_Allowance_Deduction: to check for invalid social allowance deduction
Check_Unemployment_Allowance_Deduction: to check for invalid unemployment allowance deduction
Prepare_Employee_Information_report: to prepare an excel report of employee information auditing results
Prepare_Employee_Deduction_report: to prepare an excel report of employee deduction auditing results
"""
...
def whitespace_remover(dataframe):
    ...
    # Apply strip function on selected columns
...
#     df = df.drop(df[df.DATE_OF_BIRTH == '2122-02-22'].index)
#
#     # Compute age based on current date
#     now = pd.to_datetime('now', utc=True)
#     df['AGE'] = (now.year - df['DATE_OF_BIRTH'].dt.year) - ((now.month - df['DATE_OF_BIRTH'].dt.month) < 0)
#
#     # Add 100 to age if birth year is more recent than current year
#     df.loc[df['DATE_OF_BIRTH'].dt.month > now.month, 'AGE'] += 100
#     df.loc[(df['DATE_OF_BIRTH'].dt.month == now.month) & (df['DATE_OF_BIRTH'].dt.day > now.day), 'AGE'] += 100
#
#     # Identify invalid ages
#     invalidAge = df[((df['AGE'] <= 17) | (df['AGE'] >= 65))]
#
#     # Filter by CATEGORY
#     final_results = invalidAge[(invalidAge['CATEGORY'].str.contains('Permanent|Re-Employed', na=False, case=False, regex=True))]
#
